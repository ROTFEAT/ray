...
- Ray 版本: 2.54.1, Python 3.12
- 集群规模: 248 CPU, ~700 GB 内存, 6 节点
- 无 GPU
- 结果通过 MinIO 持久化
- `--pip` 参数用**逗号**分隔多个包（如 `--pip "minio,pyarrow"`），不能用空格
...
**B. 项目基础信息**
- 读取 `CLAUDE.md`、`TODOS.md`（如果存在）
- 运行 `git log --oneline -30` 了解项目历史
- 运行 `git diff --stat` 了解当前改动
...
ls -t ~/.gstack/projects/*/*-design-*.md 2>/dev/null | head -5
# superpowers 计划
ls -t ~/.claude/plans/*.md 2>/dev/null | head -5
# 项目内计划文件
ls -t tasks/plan-*.md 2>/dev/null | head -5
```

如果找到文档，**读取最新的那个**。设计文档里的需求、数据结构、计算逻辑比口头描述更准确，以它为主要输入。

**D. 用 Grep/Glob 主动扫描相关代码**

根据用户需求（从参数、对话、设计文档中提取），用 Grep 搜索项目中相关的代码：
- 搜索和需求相关的函数名、类名、变量名
- 搜索数据库连接、文件读写、计算逻辑
- 搜索 `for` 循环、`multiprocessing`、长耗时操作——这些是并行化的候选

**E. 关键词交叉搜索设计文档**

从用户描述中提取 3-5 个关键词，跨所有设计文档搜索：
```bash
grep -li "关键词1\|关键词2\|关键词3" ~/.gstack/projects/*/*-design-*.md ~/.claude/plans/*.md 2>/dev/null
```
如果找到相关文档，读取并提取可用信息。不同用户在同一项目上的设计文档也会被发现。

**F. 综合输出**

基于以上所有信息，直接给出建议：
```
你的设计文档描述了 [需求]，项目中有 [相关代码]。
建议: [具体方案]。确认？
```

如果没有找到任何文档且无法判断需求，才问用户想做什么。

**重要：以上扫描全部静默执行，不输出任何中间过程。**
