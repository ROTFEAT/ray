...
**先自动扫描上下文，不要直接问"你想做什么"：**

1. **读取命令参数** — 用户可能在 `/ray-new` 后面写了需求描述
2. **扫描当前项目** — 读取 `CLAUDE.md`、`README.md`、`tasks/` 目录下的脚本、最近修改的 `.py` 文件，理解项目在做什么
3. **查看 git 上下文** — `git log --oneline -5` 和 `git diff --stat` 看最近在改什么
4. **检查对话上下文** — 用户之前提到的需求、脚本、数据

根据扫描结果，**主动提出建议**而不是被动提问：

```
我看到你的项目是 [项目描述]，最近在 [做什么]。
你这边有 [xxx.py] 看起来是 [功能描述]。

建议：把 [具体函数/逻辑] 并行化到集群，预计 [效果]。
要按这个方向生成 Ray 脚本吗？
```

**只有在完全无法判断时**才问用户想做什么。接受以下形式的输入：
